...
export default async function PopularMovies({ className }: PopularMoviesProps) {
	...
	return (
		<div className={className}>
			<h2 className="mb-4 text-lg font-bold">Películas populares</h2>
			<ul className="space-y-2">
				{movies.map(movie => (
					<li key={movie.id}>
						<Link
							href={`/pelicula/${movie.id}?title=${movie.title}&date=${movie.releaseDate ? getYear(movie.releaseDate.toString()) : 'N/A'}`}
