	...
	const renderPopcorn = (index: number) => {
		if (!review.rating) {
			...
			if (review.rating >= index + 1) {
				return (
					<Popcorn className="icon-thick size-5 cursor-pointer text-primary md:size-6" />
				)
			} else if (review.rating >= index + 0.5) {
				return (
					<div className="relative size-5 md:size-6">
						<div className="absolute inset-0 w-1/2 overflow-hidden">
							<Popcorn className="icon-thick size-5 text-primary md:size-6" />
						</div>
						<div className="absolute inset-0 left-1/2 w-1/2 overflow-hidden">
							<Popcorn className="icon-thick size-5 text-transparent md:size-6" />
						</div>
					</div>
				)
			} else {
				...
				<div className="flex flex-col">
					...
					<div>
						<div className="flex items-center gap-2">
							...
							{review.liked && (
								<Heart className="size-3 fill-red-500 text-red-700 dark:fill-red-600 dark:text-red-800 md:size-4" />
							)}
						</div>
