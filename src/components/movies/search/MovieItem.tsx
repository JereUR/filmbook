...
export default function MovieItem({ movie }: MovieItemProps) {
	...
	return (
		<Link href={`/pelicula/${id}?title=${title}&date=${getYear(release_date)}`}>
			<div className="flex h-full cursor-pointer items-start space-x-4 overflow-hidden rounded-2xl border p-5 shadow-lg transition-colors duration-300 ease-in-out hover:bg-card/70">
