...
export default function TitleSection({
	...
	return (
		<div className="flex items-start gap-4 md:gap-8">
			<div>
				<div className="relative h-24 w-16 shrink-0 md:h-40 md:w-28">
					<Image
						src={posterPath ? posterPath : noImagePath}
						alt={title}
						fill
							...
							{appReview ? (
								<Eye
									className={`size-5 text-foreground/70 ${
										appReview.watched ? 'fill-primary' : 'fill-white/50'
									}`}
								/>
							) : (
								<Eye
				...
				{appReview ? (
					...
					>
						<div className="mt-2 flex cursor-pointer items-center justify-center gap-1 rounded-2xl bg-card-child p-2 text-xs font-semibold hover:bg-primary">
							<MessageSquareHeart className="size-[14px]" />
