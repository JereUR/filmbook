	...
	useEffect(() => {
		if (user && reviews) {
			const foundReview = reviews.find(
				review => review.movieId === movieId && review.userId === user.id
			)
		...
		<div className="my-2 flex w-full flex-col gap-4 rounded-2xl border border-primary/50 p-2 md:my-4 md:w-1/4 md:gap-3 md:p-4">
			<div className="flex items-center justify-around gap-4">
				...
				>
					<DialogTitle>
						<CirclePlus
							className="icon-fine size-10 cursor-pointer fill-green-600 text-muted transition duration-300 ease-in-out hover:scale-110"
							onClick={() => {
			...
			<div className="flex justify-center gap-2 md:flex-col">
				...
				<ShowAppRating
					ownApp={true}
					voteAverage={
						appRating ? appRating.averageRating : rating?.averageRating
					}
					voteCount={
						appRating ? appRating.numberOfRatings : rating?.numberOfRatings
					}
